...
export default function MotivationWidget() {
  ...
  if (loading) {
    return <div className="space-y-2"><div className="h-3 bg-[#1a1a2e] w-1/4 animate-pulse" /><div className="h-4 bg-[#1a1a2e] w-full animate-pulse" /><div className="h-4 bg-[#1a1a2e] w-3/4 animate-pulse" /></div>
  }
  if (error && !line) {
    return (
      <>
<div className="hud-card-header main-drag-handle">
          <span className="text-[9px] font-mono text-[#00aaff] uppercase tracking-[0.25em]">■ SYSTEM MESSAGE</span>
        ...
        <div className="flex-1 flex flex-col justify-center">
          ...
          <button onClick={fetchLine} className="text-[10px] font-mono text-[#00aaff] hover:underline mt-1 text-left">Retry ↗</button>
        </div>
      </>
    )
  }
  ...
  return (
    <>
      ...
      <div className="flex-1 flex flex-col justify-center">
        <div className="text-[10px] font-mono text-[#a855f7] uppercase tracking-[0.15em] mb-2">DAILY DIRECTIVE &gt;</div>
        <div className="text-[14px] font-mono italic text-[#a855f7] leading-relaxed">&ldquo;{line}&rdquo;</div>
        <div className="text-[10px] font-mono text-[#4a5568] mt-2">— SYSTEM</div>
      </div>
    </>
  )
}
